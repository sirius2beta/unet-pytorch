...
def calculate_metrics(y_true, y_pred, num_classes):
    """
    y_true: 1D array (flattened ground truth)
    y_pred: 1D array (flattened prediction)
    ...
    TP = np.diag(cm)                           # True Positive (對角線)
...
if __name__ == "__main__":
    ...
    video_save_path = "video_out/v2_convert.MP4"
    video_fps       = 30.0
    #----------------------------------------------------------------------------------------------------------#
    ...
    if mode == "predict":
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                ...
                # 先推論！！
                r_image, pred_mask = unet.detect_image(image, return_mask=True)

                # 讀取 ground truth
                gt_mask_path = image_path.replace("img", "mask").replace(".jpg", ".png")  # 這行依照你的實際資料夾名字
                gt_mask = Image.open(gt_mask_path)

                # 🔥這時 pred_mask 已經有了，所以現在可以 resize
                gt_mask = gt_mask.resize(pred_mask.shape[::-1], Image.NEAREST)
        ...
        calculate_metrics(all_gt, all_pred, num_classes=len(name_classes))
